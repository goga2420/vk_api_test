...
class HtmlPostsExporter:
    ...
    def export(self):
        ...
        string_end = '</body>\n</html>'
        html_string = head + self.all_posts_link_maker(self) + string_end
    ...
    def all_posts_link_maker(self):
        ...
        for i in range(0, len(self.posts)):
           all_links = all_links + self.one_post_link_maker(self, self.posts[i])

        return all_links

    def one_post_link_maker(self, post):
        vk_url = 'https://vk.com/wall'
        href = '\t<a href="' + vk_url + str(self.wall_id) + '_' + str(post['id']) + '">' + str(
            TopPostsSelector(0, 0).extract_likes(post)) + '❤</a><br/>\n'
